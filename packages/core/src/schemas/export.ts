...
export const exportSchemaToCode = async (
  ...
  indexCssFile.path = targetPath ? path.join(targetPath, indexCssFile.path) : indexCssFile.path
  ...
  viewsEntryTsFile.path = targetPath
    ? path.join(targetPath, viewsEntryTsFile.path)
    : viewsEntryTsFile.path
  viewsEntryTsFile.content = `
import { useAppMountFunc } from "@vue-cook/render";
${viewSchemaFiles
  .map((viewSchemaFile, index) => {
    let realtivePath = './' + path.relative(path.dirname(viewsEntryTsFile.path), path.dirname(viewSchemaFile.path))
...
`
  ...
  indexTsFile.path = targetPath ? path.join(targetPath, indexTsFile.path) : indexTsFile.path
